...
#         print(y_test_used.shape,"\n")
        ...
        list_n_estimators = [1, 5, 10, 15, 20]
        list_max_depths = range(1, 33, 2)
        # list_max_depths = range(1, 33)
        list_max_features = [1, 3, 5, 7, 9]
        # list_max_features = [0.1, 0.3, 0.5, 0.7, 0.9]
        list_min_samples_splits = [0.1, 0.3, 0.5, 0.7, 0.9, 1.0]
        list_min_samples_leafs = [0.1, 0.3, 0.5]
        
        min_mse = 1
        for estimator_val in list_n_estimators:
            for depth_val in list_max_depths:
                for feature_val in list_max_features:
                    for split_val in list_min_samples_splits:
                        for leaf_val in list_min_samples_leafs: 
                            model = RandomForestRegressor(n_estimators = estimator_val, max_features = feature_val, max_depth = depth_val, min_samples_split = split_val, min_samples_leaf = leaf_val)
                            ...
                            print("Test_subj {} Est: {} Depth: {} Max_features: {} Split:{} Leaf {} All_MSE: {} All_R2: {}".format(test_person_num, estimator_val, depth_val, feature_val, split_val, leaf_val, all_mse, all_r2_score))

                            if all_mse < min_mse:
                                min_mse = all_mse
                                best_K = estimator_val 
                                best_Depth = depth_val
                                best_Split = split_val
                                best_Leaf = leaf_val
                                best_Features = feature_val
                                print("The best Est is: {} The best Depth is: {} The best Max_features: {} The best Split: {} The best Leaf: {} The best mse: {}".format(best_K, best_Depth, best_Features, best_Split, best_Leaf, all_mse))

        regression = RandomForestRegressor(n_estimators = best_K, max_depth = best_Depth, max_features = best_Features, min_samples_split = best_Split, min_samples_leaf = best_Leaf)
...
#         plt.show()
        ...
        # read_book = open_workbook("RF_allcon_win3_revised.xls")
        ...
        # row.write(idx_run+list_row[test_person_num]+2,3, best_Depth)
        # row.write(idx_run+list_row[test_person_num]+2,4, best_Split)
        # row.write(idx_run+list_row[test_person_num]+2,5, best_Leaf)
        ...
        # row.write(idx_run+list_row[test_person_num]+2,23, best_Features)
        # write_book.save("RF_allcon_win3_revised.xls")
